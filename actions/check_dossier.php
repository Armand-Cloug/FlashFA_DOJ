...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'], $_POST['statut'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT statut FROM dac WHERE id = :id");
        $stmt->execute([':id' => $id]);
        $ancien_statut = $stmt->fetchColumn();

        if (!$ancien_statut) {
            http_response_code(404);
            exit('DAC introuvable.');
        }

        // ✅ Mise à jour du statut du DAC
        $query = "UPDATE dac SET statut = :statut WHERE id = :id";
        ...
        header('Location: ../private/dossier.php?success=Statut mis à jour');
        exit();
    } catch (PDOException $e) {
        error_log("Erreur lors de la mise à jour du DAC : " . $e->getMessage());
