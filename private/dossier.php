...
$query = "SELECT * FROM dac";
$result = $pdo->query($query);
?>

<body class="dossier-body">
    <div class="main-content">
        <h1>📂 Gestion des DAC</h1>

        <!-- ✅ Barre de recherche -->
        <input type="text" id="searchBar" placeholder="Rechercher une affaire..." onkeyup="searchDossier()">

        <button class="btn-add" onclick="window.location.href='/private/ajouter_dossier.php'">+ Ajouter un dossier</button>
        ...
        <table class="dossier-table">
            ...
            <tbody>
                <?php while ($row = $result->fetch(PDO::FETCH_ASSOC)): ?>
                    <tr>
                        <td>
                            <a href="<?= htmlspecialchars($row['lien_dac']); ?>" target="_blank" class="affaire-link">
                                <?= htmlspecialchars($row['nom_affaire']); ?>
                            </a>
                        </td>
                        <td><?= htmlspecialchars($row['responsable'] ?? 'Non attribué'); ?></td>
                        <td>
                            <form method="POST" action="/actions/check_dossier.php">
                        ...
                        <td>
                            ...
                            if ($statut === 'Transmis au Juge' || $statut === 'Affaire close') {
                                ...
                                if ($dateCreation) {
                                    ...
                                    $dateFin->modify('+3 days'); // Ajoute 3 jours à la date de création
                                    $now = new DateTime();
                        ...
                        <td>
                            <a href="<?= htmlspecialchars($row['lien_dc']); ?>" target="_blank" class="btn-public">Dossier Complet</a>
                            <a href="/private/update_dossier.php?id=<?= $row['id']; ?>" class="btn-edit">Modifier</a>
                        </td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
